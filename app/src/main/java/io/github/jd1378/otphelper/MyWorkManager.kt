...
object MyWorkManager {
  fun doDataMigration(context: Context): Operation {
    ...
    return WorkManager.getInstance(context)
        .enqueueUniqueWork(migrateWorkName, ExistingWorkPolicy.KEEP, migrateWork)
  }
  ...
  fun enableHistoryCleanup(context: Context): Operation {
    val cleanupWorkRequest =
        PeriodicWorkRequestBuilder<DataCleanupWorker>(1, TimeUnit.DAYS)
            ...
            .setInitialDelay(getTimeToMidnightMillis(), TimeUnit.MILLISECONDS)
            .build()
    return WorkManager.getInstance(context)
  ...
  fun schedulePersistenceWatchdog(context: Context): Operation {
    val request =
        PeriodicWorkRequestBuilder<PersistenceWatchdogWorker>(15, TimeUnit.MINUTES)
            .setInitialDelay(15, TimeUnit.MINUTES)
            .build()
    return WorkManager.getInstance(context)
        .enqueueUniquePeriodicWork(
            persistenceWatchdogWorkName,
            ExistingPeriodicWorkPolicy.UPDATE,
            request,
        )
  }
  ...
  fun rebindListeners(context: Context, silent: Boolean = false): Operation {
    val data = workDataOf("silent" to silent)
    val request = OneTimeWorkRequestBuilder<RebindListenersWorker>().setInputData(data).build()
    // REPLACE matters on OEM firmware: a stale or stopped KEEP request must not suppress repair.
    return WorkManager.getInstance(context)
        .enqueueUniqueWork(rebindListenersWorkName, ExistingWorkPolicy.REPLACE, request)
  }
}
